...
install() {
    install_base
    bash <(curl -L -s https://github.com/yimshing/sprov-ui/blob/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
install_bbr() {
    bash <(curl -L -s https://github.com/yimshing/sprov-ui/blob/master/bbr.sh)
    if [[ $? == 0 ]]; then
        echo ""
...
update_shell() {
    wget -O /usr/bin/sprov-ui -N --no-check-certificate https://github.com/yimshing/sprov-ui/blob/master/sprov-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
